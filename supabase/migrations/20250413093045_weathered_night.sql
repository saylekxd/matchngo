/*
  # Fix database schema for NGO-Expert Connect

  1. Changes
    - Add conditional checks before creating types and constraints
    - Ensure all objects are created only if they don't already exist
    - Maintain all the same tables, policies, and triggers from previous migrations
...
BEGIN
  ...
  EXECUTE 'ALTER TABLE IF EXISTS ngo_profiles ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS expert_profiles ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS opportunities ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS applications ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS messages ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS reviews ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS saved_opportunities ENABLE ROW LEVEL SECURITY';
END
$$;
